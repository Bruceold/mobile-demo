...
<script>
  ...
  import {CATEGORY_CONTENT_KEY,CATEGORY_CONTENT_TIME_INTERVAL} from './config.js';
  export default{
    ...
    methods: {
      getCotent(id){
      	...
      	if (contents && contents[id]) {
      	  ...
      	  if (curTime -  updateTime <= CATEGORY_CONTENT_TIME_INTERVAL) {
      	  	return this.getCotentByLocalStorage(contents[id]);
      	  } else {
      ...
      getCotentByLocalStorage(content) {
